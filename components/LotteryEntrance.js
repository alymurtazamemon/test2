...
export default function LotteryEntrance() {
    ...
    return (<div>
            ...
            <button onClick={async function() {await enterRaffle(onSuccess: handleSuccess, onError: (error) => console.log(error)</div>)}}>Enter Raffle</button>
            {raffleAddress ? <div>Entrance Fee: {entranceFee} ETH</div>:<div>No raffle address detected!</div>}
        
        
        </div>)  // as entranceFee is global variable we can use it in any block, so we are using it here too
}
